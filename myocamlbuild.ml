(* OASIS_START *)
(* DO NOT EDIT (digest: 779b6b20fb9307d777129a907a80e0d1) *)
...
let package_default =
  {
     MyOCamlbuildBase.lib_ocaml = [("lib/spf", ["lib"])];
     lib_c = [("spf", "lib", [])];
     flags =
       [
          (["oasis_library_spf_ccopt"; "compile"],
            [
               (OASISExpr.EBool true,
                 S [A "-ccopt"; A "-Wall"; A "-ccopt"; A "-Werror"])
            ]);
          (["oasis_library_spf_cclib"; "link"],
            [(OASISExpr.EBool true, S [A "-cclib"; A "-lspf2"])]);
